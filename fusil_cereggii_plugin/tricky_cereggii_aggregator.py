...
def _try_import_and_get_attribute(module_name: str, attribute_name: str) -> object | None:
    ...
    try:
        ...
        print(
            f"[Cereggii Plugin] ERROR: '{attribute_name}' not found in {module_name}",
            file=sys.stderr,
        )
        return None
    except Exception as e:
...
print("[Cereggii Plugin] Finished aggregating code snippets.", file=sys.stderr)
...
_EXPORT_MAP = {
    ...
    "tricky_colliding_keys": [("colliding_key_sets", "colliding_key_sets", "var_name")],
    "tricky_weird_cereggii": [
            ...
            try:
                if extraction_type == "dict_keys":
                    if isinstance(attribute_value, dict):
                        ...
                        print(
                            f"  [Cereggii Plugin] + Aggregated {len(list(attribute_value.keys()))} names from {module_name}.{attr_name}",
                            file=sys.stderr,
                        )
                    else:
                        print(
                            f"  [Cereggii Plugin] - WARNING: Expected dict for '{attr_name}', got {type(attribute_value)}",
                            file=sys.stderr,
                        )
                    ...
                    print(
                        f"  [Cereggii Plugin] + Exported variable name '{attr_name}' as '{target_var_or_list}'",
                        file=sys.stderr,
                    )

            except Exception as e:
                print(
                    f"  [Cereggii Plugin] - ERROR processing {module_name}.{attr_name}: {e}",
                    file=sys.stderr,
                )
        else:
            print(
                f"  [Cereggii Plugin] - Failed to retrieve {module_name}.{attr_name}",
                file=sys.stderr,
            )
...
print(
    f"[Cereggii Plugin] AtomicInt64 Instances: {len(tricky_atomicint64_instance_names)}",
    file=sys.stderr,
)
print(
    f"[Cereggii Plugin] AtomicDict Instances: {len(tricky_atomicdict_instance_names)}",
    file=sys.stderr,
)
...
print(
    f"[Cereggii Plugin] Weird Cereggii: {len(tricky_weird_cereggii_instance_names)}",
    file=sys.stderr,
)
...
print(
    f"[Cereggii Plugin] Python Utils Scenarios: {len(python_utils_scenario_names)}", file=sys.stderr
)
print(
    f"[Cereggii Plugin] ThreadHandle Scenarios: {len(threadhandle_scenario_names)}", file=sys.stderr
)
print(f"[Cereggii Plugin] Stateful Scenarios: {len(stateful_scenario_names)}", file=sys.stderr)
print(
    f"[Cereggii Plugin] Concurrency Hell: {len(concurrency_hell_scenario_names)}", file=sys.stderr
)
